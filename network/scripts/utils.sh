...
setGlobals() {
  ...
  if [ "$ORG" == 'consumer' ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/consumer.pharma-network.com/distributor/Admin@consumer.pharma-network.com/msp
    if [ "$PEER" -eq 0 ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/distributor.pharma-network.com/distributor/Admin@distributor.pharma-network.com/msp
    if [ "$PEER" -eq 0 ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/manufacturer.pharma-network.com/manufacturer/Admin@manufacturer.pharma-network.com/msp
    if [ "$PEER" -eq 0 ]; then
      CORE_PEER_ADDRESS=peer0.manufacturer.pharma-network.com:9051
    fi
    if [ "$PEER" -eq 1 ]; then
      CORE_PEER_ADDRESS=peer1.manufacturer.pharma-network.com:10051
    fi
  elif [ "$ORG" == 'retailer' ]; then
    CORE_PEER_LOCALMSPID="retailerMSP"
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_RETAILER_CA
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/retailer.pharma-network.com/retailer/Admin@retailer.pharma-network.com/msp
    if [ "$PEER" -eq 0 ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/transporter.pharma-network.com/transporter/Admin@transporter.pharma-network.com/msp
    if [ "$PEER" -eq 0 ]; then
  ...
  #   CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/upgrad.pharma-network.com/distributor/Admin@upgrad.pharma-network.com/msp
  #   if [ "$PEER" -eq 0 ]; then
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.pharma-network.com:7050 -C "$CHANNEL_NAME" -n regnet -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.pharma-network.regnet:instantiate"]}' -P "OR ('consumerMSP.member','distributorMSP.member', 'manufacturerMSP.member','retailerMSP.member', 'transporterMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.pharma-network.com:7050 --tls "$CORE_PEER_TLS_ENABLED" --cafile $ORDERER_CA -C $CHANNEL_NAME -n regnet -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.pharma-network.regnet:instantiate"]}' -P "OR ('consumerMSP.member','distributorMSP.member','manufacturerMSP.member','retailerMSP.member', 'transporterMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode upgrade -o orderer.pharma-network.com:7050 -C $CHANNEL_NAME -n regnet -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.pharma-network.regnet:instantiate"]}' -P "OR ('consumerMSP.member','distributorMSP.member','upgradMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode upgrade -o orderer.pharma-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n regnet -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.pharma-network.regnet:instantiate"]}' -P "OR ('consumerMSP.member','distributorMSP.member','upgradMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
chaincodeQuery() {
  ...
  do
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n regnet -c '{"Args":["org.pharma-network.regnet:instantiate"]}' >&log.txt
    res=$?
    set +x
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.pharma-network.com:7050 -C $CHANNEL_NAME -n regnet $PEER_CONN_PARMS -c '{"Args":["org.pharma-network.regnet:createStudent","0001","Aakash Bansal","connect@aakashbansal.com","15"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.pharma-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n regnet $PEER_CONN_PARMS -c '{"Args":["org.pharma-network.regnet:createStudent","0001","Aakash Bansal","connect@aakashbansal.com"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
