...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"pharma-network\")"
  ...
  echo
  echo "	fabricNetwork.sh generate -c pharma-network"
  echo "	fabricNetwork up -c pharma-network -s couchdb"
  echo "        fabricNetwork up -c pharma-network -s couchdb -i 1.4.0"
  echo "	fabricNetwork up -l node"
  echo "	fabricNetwork down -c pharma-network"
  echo
...
function generateChannelArtifacts() {
  ...
  # echo "#################################################################"
  # set -x
  # configtxgen -profile pharma-network -outputAnchorPeersUpdate ./channel-artifacts/upgradMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg upgradMSP
  # res=$?
  # set +x
  # if [ $res -ne 0 ]; then
...
CLI_DELAY=5
# channel name defaults to "pharma-network"
