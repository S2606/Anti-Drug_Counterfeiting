...
async function main(drugName, serialNo){
    try {
        ...
        console.log('...... Fetching Drug State');
        const drugBuffer = await pharmanetContract.submitTransaction('viewDrugCurrentState', 
        drugName, serialNo);

        //process response
        console.log('...... Processing the Drug State \n\n');
        let newdrug = JSON.parse(drugBuffer.toString());
        console.log(newdrug);
        console.log('\n\n......Drug State Fetch complete!');
        return newdrug;
    } catch (error) {
		console.log(`\n\n${error}\n\n`);
